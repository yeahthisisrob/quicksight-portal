...
The catalog leads with what SMUS does not have: **every calculated field in the account**, one row per distinct expression, with where it is defined, the datasets and SMUS listings it lives on, how many dashboards, analyses and visuals use it, and a conflict badge wherever one name carries several expressions. One click opens the lineage both ways: the columns it reads, each tied to its SMUS column with description and glossary terms, the calculated fields it builds on, and everything that reads it down to the visual, with the variants side by side. A columns tab ties plain columns to their SMUS column, matching through a rename (case, separators and camel humps carry no meaning) and saying which of the two things went wrong when it cannot: a dataset that matched no listing, a listing whose schema does not have the column, or a listing that publishes no column list at all. The SMUS assets tab shows the published listings per project. SMUS owns the business metadata and the portal shows it read-only: glossary terms, metadata forms, descriptions, the Glue table and its columns. The portal adds what only QuickSight knows:
...
![Catalog: plain columns tied back to the SMUS column they came from](docs/screenshots/catalog-columns.png)
