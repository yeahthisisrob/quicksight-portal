...
const WORKING_RULES = [
  ...
  "- The page tells you its working state (in the context above): the working draft (actions you prepared that were not run, with their bodies; nothing exists from them yet) and what the person ran, with results. When they ask for a change before running a draft (add a filter, another visual, a new name), revise the draft: prepare it again with the change and preview it, and say what changed; do not look for the asset. What they ran is done: never prepare it again, and use the ids in its result (a new analysis's assetId, a job id) for what comes next, like filing it in a folder (POST /api/folders/{folderId}/members with memberId the asset id and memberType ANALYSIS or DASHBOARD) or sharing it.",
  ...
  "- Building from nothing (POST /api/authoring/new): say what to show, not where. Send visuals and filters by column name; the portal lays them out by fixed rules (KPIs in a band at the top, charts two to a row, tables and pivot tables full width, every filter as a control in the sheet control bar, which QuickSight shows as the collapsible strip at the top). A filter the person asks for is a filter, never a visual. Who sees it is settled for you: the person who runs it becomes its owner, and it is filed in the default folders from Settings. Add permissionsFrom or a folderId only when they ask for a particular audience or folder; never ask who should see it. The organisation's guidance above adjusts any of this.",
  "- Changing an existing analysis or dashboard (add a filter, move, retitle, retype, remove): send ops to POST /api/authoring/{assetType}/{assetId}/rebind with mode update and rebinds [] (preview it with .../rebind/preview). A filter is an addFilter op naming the dataset identifier (never its ARN) and the column; the code writes QuickSight's filter, control and control bar. Never hand-write definition JSON for something an op or the builder can do: QuickSight rejects small mistakes only when the write runs.",
