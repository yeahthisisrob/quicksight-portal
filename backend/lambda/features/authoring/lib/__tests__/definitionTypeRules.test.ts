...
describe('currentEditableType', () => {
  it('tells bars from columns and pies from donuts', () => {
    expect(
      currentEditableType('BarChartVisual', { ChartConfiguration: { Orientation: 'VERTICAL' } })
    ).toBe('ColumnChart');
    expect(currentEditableType('BarChartVisual', {})).toBe('BarChart');
    expect(
      currentEditableType('PieChartVisual', {
        ChartConfiguration: { DonutOptions: { ArcOptions: { ArcThickness: 'MEDIUM' } } },
      })
    ).toBe('DonutChart');
...
describe('applyTypeRules', () => {
  ...
  it('refuses unknown types and skips a rule that changes nothing', () => {
    expect(() =>
      applyTypeRules(sampleDefinition(), { chartFamily: [{ from: 'Bar' as any, to: 'Table' }] })
    ).toThrow('must be one of');
    expect(
      applyTypeRules(sampleDefinition(), { chartFamily: [{ from: 'Table', to: 'Table' }] }).changes
    ).toEqual([]);
  });
  ...
  it('turns gauges into KPIs and gives every KPI the template options', () => {
            ...
            Values: [
              {
                MeasureField: {
                  NumericalMeasureField: {
                    FieldId: 'f',
                    Column: { DataSetIdentifier: 'orders', ColumnName: 'revenue' },
                  },
                },
              },
            ],
            TargetValues: [
              {
                MeasureField: {
                  NumericalMeasureField: {
                    FieldId: 't',
                    Column: { DataSetIdentifier: 'orders', ColumnName: 'cost' },
                  },
                },
              },
            ],
    ...
    const { definition, changes } = applyTypeRules(
      source,
      { kpi: true },
      { templateKpiOptions: options }
    );

    const kpis = definition.Sheets[0].Visuals.filter((w: any) => w.KPIVisual).map(
      (w: any) => w.KPIVisual
    );
    expect(kpis.map((k: any) => k.VisualId)).toEqual(['v2', 'g1']);
    expect(
      kpis.every((k: any) => k.ChartConfiguration.KPIOptions?.Sparkline?.Type === 'AREA')
    ).toBe(true);
...
describe('casts', () => {
  ...
  it('plans a cast field and a rename for every column whose type changed, and rewrites the references', () => {
    ...
    expect(plan.addCalculatedFields).toEqual([
      ...
      {
        identifier: 'orders',
        name: 'Order Date_as_datetime',
        expression: 'parseDate({Order Date})',
      },
    ]);
    expect(plan.renames.get('orders')).toEqual({
      revenue: 'revenue_as_decimal',
      'Order Date': 'Order Date_as_datetime',
    });
    expect(plan.warnings).toEqual([
      expect.stringContaining('note was DATETIME and is now DECIMAL'),
    ]);
