...
export function processMappings(mappings: any[], termId: string) {
  ...
  const visualFieldMappings = termMappings.filter((m) => m.fieldId?.startsWith('visual-field:'));
...
export function mapVisualFields(mappings: any[], visualFields: any[]) {
  return mappings.map((mapping) => {
    const field = visualFields.find(
      (vf) =>
        vf.visualFieldId === mapping.fieldId || `visual-field:${vf.displayName}` === mapping.fieldId
    );
    return { ...mapping, field };
  });
}
