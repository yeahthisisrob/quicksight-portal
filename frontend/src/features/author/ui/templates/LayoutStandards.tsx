 ...
 * and controls a new asset can be built on. Tagged with the star in the
 * Editor or with the quicksight-portal:template tag anywhere.
 */
  ...
  if ((standards.data?.length ?? 0) === 0) {
    return (
      <EmptyState
        ...
        description="Star a dashboard or analysis in the Editor to make it a standard: new assets can take its layout, text, controls and theme."
      />
    );
  }
  return (
      ...
      {standards.data?.map((s) => (
        <Stack key={`${s.type}:${s.id}`} direction="row" spacing={1} sx={{ alignItems: 'center' }}>
          ...
          <Button
            ...
            to={`/author?tab=studio&type=${s.type}&id=${encodeURIComponent(s.id)}`}
