...
export default function TagsCell({ tags = [], onClick }: TagsCellProps) {
  ...
  const sortedTags = [...tags].sort((a, b) => {
    const aIsPortal =
      a.key === PORTAL_EXCLUDE_TAGS.EXCLUDE_FROM_CATALOG ||
      a.key === PORTAL_EXCLUDE_TAGS.EXCLUDE_FROM_PORTAL;
    const bIsPortal =
      b.key === PORTAL_EXCLUDE_TAGS.EXCLUDE_FROM_CATALOG ||
      b.key === PORTAL_EXCLUDE_TAGS.EXCLUDE_FROM_PORTAL;
  ...
  return (
    ...
    >
      {displayTags.map((tag, index) => {
        ...
        const chipType = isExcludeFromCatalog
          ? 'CATALOG_HIDDEN'
          : isExcludeFromPortal
            ? 'PORTAL_HIDDEN'
            : 'TAG';

        const label = isExcludeFromCatalog
          ? 'Catalog Hidden'
          : isExcludeFromPortal
            ? 'Portal Hidden'
            : tag.value.length > 15
              ? `${tag.value.substring(0, 15)}...`
              : tag.value;
