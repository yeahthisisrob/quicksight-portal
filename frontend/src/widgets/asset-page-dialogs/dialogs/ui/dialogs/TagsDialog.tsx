...
import type { TagsDialogProps } from '../../model';
...
const TagRow = ({
  ...
  const isPortalTag =
    tag.key === PORTAL_EXCLUDE_TAGS.EXCLUDE_FROM_CATALOG ||
  ...
  return (
    <>
      ...
      {!isLast && <Box sx={{ height: 1, backgroundColor: 'divider', mx: 1 }} />}
    </>
  );
};
        ...
        <Grid size={{ xs: 12, sm: 6 }}>
          <Box
            sx={{
              p: 1.5,
              borderRadius: `${borderRadius.sm}px`,
              backgroundColor: hasCatalogExclusion
        ...
        <Grid size={{ xs: 12, sm: 6 }}>
          <Box
            sx={{
              p: 1.5,
              borderRadius: `${borderRadius.sm}px`,
              backgroundColor: hasPortalExclusion
...
export default function TagsDialog({
  ...
  const handleSave = async () => {
    try {
      ...
      enqueueSnackbar(error instanceof Error ? error.message : 'Failed to save tags', {
        variant: 'error',
      });
    } finally {
      setSaving(false);
    ...
    >
      ...
      <DialogContent sx={{ p: 3 }}>
        ...
        {resourceType === 'folder' && editMode && (
          <Box sx={{ mb: 3 }}>
            <PortalVisibilityControls tags={tags} onToggleTag={handleTogglePortalTag} />
          </Box>
        )}
        ...
        >
          <Box
            sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: tags.length > 0 ? 1.5 : 0 }}
          >
            <TagIcon
            ...
            {tags.length > 0 && (
              <TypedChip type="TAG" count={tags.length} showIcon={false} size="small" />
            )}
          </Box>
        ...
        {editMode && (
          <Box sx={{ mt: 3 }}>
            <Typography
              variant="subtitle2"
              sx={{ mb: 1.5, fontWeight: typography.fontWeight.semibold }}
            >
              Add New Tag
            </Typography>
